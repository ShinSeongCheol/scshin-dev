...
export default function PostCard({id, title, content, createdAt, thumbnailUrl}: Post) {
    ...
    return (
        <Link href={`/posts/${id}`} className={''}>
            <article className="group flex flex-col h-full bg-white rounded-2xl overflow-hidden border border-gray-200 transition-all duration-300 cursor-pointer hover:transform hover:-translate-y-1 hover:shadow-2xl hover:border-slate-400">
